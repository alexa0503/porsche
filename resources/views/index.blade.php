...
<div class="abs zpzl" id="photo">
  <div class="abs photo-parent" style="width:100%;height:100%;"><img id="photo-img"  src="" style="pointer-events:none;display:none;" /></div>
...
$().ready(function(){
  ...
  //照片上传
  $('.sczpclick').on('touchend', function(){
    upload('local');
    return;
    wx.chooseImage({
      count: 1, // 默认9
  ...
  $('.qd_click').on('touchend', function(){
    var url = '/cartoon';
    var _x = 0;
    var _y = 0;
    var _obj = img.offset();
    _x = -1*(_obj.left - (mWidth/2 - 410*screenScale/2));
    _y = -1*(_obj.top - 22*screenScale);
    //_y = -1*(_obj.top - (mHeight - nHeight) / 2 - 22*screenScale);
    //console.log(_x.toFixed(2),_y.toFixed(2),screenScale)
    var data = {
      x:(_x/screenScale).toFixed(2),
      y:(_y/screenScale).toFixed(2),
      scale:currentScale,
      w:410,
      h:600,
      _token:window.Laravel.csrfToken
    }
    console.log(_obj.left,data);
    //return;
  ...
  function upload(serverId)
  {
    ...
    $.ajax({
      ...
      if (json.ret == 0){
        ...
        var _s = 410/nWidth;
        if( nWidth / nHeight > 600/410 ){
          _s = 600/nHeight;
        }
        minScale = _s*screenScale;
        nWidth = nWidth*screenScale;
        nHeight = nHeight*screenScale;
        scale = _s;
        currentScale = 1;
        liveScale = 1;
        deltaX = 0;
        ...
        //照片居中
        var _l = (mWidth - nWidth) / 2
        var _t = (mHeight - nHeight - 22*screenScale) / 2
        img.offset({
          left: _l,
          top: _t
        });
